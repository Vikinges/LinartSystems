...
- Деплой одной командой (Windows): `.\scripts\deploy.ps1 -Message "..."` — коммитит всё, пушит, дёргает webhook и ждёт перезапуска (проверяет version/uptime в /health).
- Деплой вручную: после push можно дергать webhook Portainer (обновляет стек): `curl -k -X POST https://lsc-led.de:9443/api/stacks/webhooks/b903907f-2110-4593-963b-e0f71779e249` (флаг `-k` обязателен — самоподписанный сертификат Portainer). Старый адрес port.linart.club больше не действует (июнь 2026).
