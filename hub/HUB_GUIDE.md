...
5. Для локальной проверки добавить в `C:\Windows\System32\drivers\etc\hosts` строку:
   ```
   127.0.0.1 hub.linart.club
   ```
6. Открыть `https://hub.linart.club/` и принять предупреждение о самоподписанном сертификате (при первом запуске).
...
5. Проверить в браузере: на главной странице появится карточка нового сервиса, а по адресу `https://hub.linart.club/service3/` — его приложение.
...
- Доступ: `https://hub.linart.club/admin` (по умолчанию через всплывающее окно на главной странице).
- Возможности:
