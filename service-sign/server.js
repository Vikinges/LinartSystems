...
const stmtInsertJob = db.prepare(`
  INSERT INTO sign_jobs (
    id, token_hash, created_at, expires_at, status, source_path, original_name
  ) VALUES (
    @id, @token_hash, @created_at, @expires_at, @status, @source_path, @original_name
  )
`);
    ...
    body: `
      ...
      <div class="grid">
        <section class="card">
          ...
          <div class="pdf-frame">
            <iframe src="/s/${encodeURIComponent(token)}/pdf" title="Document preview"></iframe>
          </div>
          <div class="footer">
            <a class="btn-link" href="/s/${encodeURIComponent(token)}/pdf" target="_blank" rel="noopener">Open PDF in new tab</a>
          </div>
        </section>
        <section class="card">
  ...
  return renderLayout({
    ...
    body: `
      ...
      <section class="card">
        ...
        <a class="btn btn-primary" href="/s/${encodeURIComponent(token)}/download">Download signed PDF</a>
      </section>
    `,
  });
}
...
app.get('/s/:token/pdf', rateLimit, (req, res) => {
  const token = req.params.token;
  const job = getJobByToken(token);
  noStore(res);
  if (!job) {
...
app.get('/s/:token/download', rateLimit, (req, res) => {
  const token = req.params.token;
  const job = getJobByToken(token);
  noStore(res);
...
app.post('/s/:token/submit', rateLimit, async (req, res) => {
  const token = req.params.token;
  const job = getJobByToken(token);
  noStore(res);
