...
# optionally set HOST_URL if the service sits behind a proxy
```
...
The rendered form shows an **Admin tools** card (bottom of the page). Default credentials are `admin` / `admin`. After the first login you should change the password immediately. From the admin card you can:

- Upload additional PDF templates. Files are stored under `public/templates/` and tracked in `data/templates.json`.
- Switch between any uploaded/built-in template; the currently active template is highlighted and the server logs each change.
- Change the admin password without touching the server filesystem.

All template metadata (filename, upload date, size, source) is recorded in `data/templates.json`. Each change of the active template updates the manifest and takes effect for the next submission without restarting the service.
